...
Dir=$(ls -d 04_RAD_DEMULTIPLEXED/RAD*'_demultiplexed_new'/*/*/ | sed -n ${SLURM_ARRAY_TASK_ID}p)

Sample=$(echo $Dir | cut -d'/' -f3 | cut -d'.' -f1)

Sam=$(echo $Sample | cut -d'_' -f1,2)

RAD=$(echo $Dir | cut -d'/' -f2 | cut -d'_' -f1)

Isolate=$(echo $Dir | cut -d'/' -f3)

Genome=$(ls -d ../ref/$Isolate)

mkdir 05_RAD_MAPPED/$RAD'_BWAmap'/

F1=$(ls $Dir/$Sample.1.fq)

F2=$(ls $Dir/$Sample.2.fq)

bwa mem -t 10 $Genome/$Isolate $F1 $F2 > 05_RAD_MAPPED/$RAD'_BWAmap'/$Sample.sam

samtools view -b -r $Sam -@ 10 05_RAD_MAPPED/$RAD'_BWAmap'/$Sample.sam > 05_RAD_MAPPED/$RAD'_BWAmap'/$Sample.bam

Sam=$(ls 05_RAD_MAPPED/$RAD'_BWAmap'/$Sample.bam)

mkdir 05_RAD_MAPPED/SAM_files/SORTED/

samtools sort $Sam -@ 5 -m 2G -o 05_RAD_MAPPED/SAM_files/SORTED/$Sample'_sorted.bam'
