...
# Demultiplex and sort reads by sample
	...
	# To properly name files you need a barcode file that looks like this:
		...
		# ACTTGAAC        sample3
		# etc...
Vcf=$(ls 07_RAD_FREEBAYES_COMMON_VCF/*/*.vcf | sed -n ${SLURM_ARRAY_TASK_ID}p)

Samples=$( echo $Vcf | cut -d'/' -f3 | cut -d'.' -f1 | uniq)

Gen=$(echo $Vcf | cut -d'/' -2)
...
for vcf in $(ls 07_RAD_FREEBAYES_COMMON_VCF/$Gen/$Samples.vcf)
	do sample=$(echo $vcf | cut -d'/' -f3 | cut -d '.' -f1)
	awk 'BEGIN{ FS=OFS="\t" }{ print $1,$2,$4,$8,$9,$10 }' $vcf | cut -d ';' -f1,8,41|tr -s ';' '\t'| cut -d'=' -f1,3,4 | tr -s '=' '\t' | cut -d $'\t' -f 1,2,3,7,9| cut -d ':' -f1,2,4,6| tr -s ':' '\t'| cut -d $'\t' -f 1,2,3,4,6,7,8|awk -F'\t' '{gsub(/[,]/, "", $4)}1' OFS='\t'| cut -d',' -f1,2 | tr -s ',' '\t' | awk '!/^#/ {print}' | awk 'BEGIN{FS=OFS="\t"}{a1=($6/$5);a2=($7/$5);a3=($8/$5); print $1,$2,$3,$4,$5,$6,0+$7,0+$8}' >> 07_RAD_FREEBAYES_COMMON_VCF/$Gen/$sample'.'allfreq.txt
done
#for zip in $(ls 06_RAD_SNPS_FREEBAYES/A5/$Samples'_'*.vcf.gz)
#	do tabix -pvcf -f $zip
#done

#isol=$(ls 06_RAD_SNPS_FREEBAYES/A5/$Samples'_'*.vcf.gz | tr '\n' ' ')
#n=$(ls 06_RAD_SNPS_FREEBAYES/A5/$Samples'_'*.vcf.gz | wc -l)
#num=$(echo "scale=0;$n*75/100" | bc )

#echo $Samples' '$n' '$num

#bcftools isec --collapse all -o 07_RAD_FREEBAYES_COMMON_VCF/A5/$Samples'_common' -w1 -O v -n +$n $isol
